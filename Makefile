...
# The briefings, packed five bits a character into bank 7 (tools/packtext.py);
# src/game/briefings.asm is the authored text and is not assembled itself.
TEXT_GEN  := $(GEN_DIR)/briefings_packed.asm
...
# MINI.BIN: the vortex chase on its own, on the same disc -- src/mini.asm and
# src/minidisc.asm are the game's two sources assembled again with MINI_ONLY
# set, and everything they write goes under build/mini/ so the game's own
# images and symbol file are never touched. RUN"MINI to play it.
MINI_DIR   := $(BUILD_DIR)/mini
MINI_MAIN  := $(SRC_DIR)/mini.asm
...
$(GAME_RAW) $(SPRITE_RAW) $(LIB_RAW) $(SYM) &: $(ASM_SOURCES) $(TABLES) $(SPRITES) $(MUSIC_GEN) $(HUD_ICONS) $(TEXT_GEN) | $(BUILD_DIR)
...
$(MINI_RAW) $(MINI_SPRITE_RAW) $(MINI_SYM) &: $(ASM_SOURCES) $(TABLES) $(SPRITES) $(MUSIC_GEN) $(HUD_ICONS) $(TEXT_GEN) | $(MINI_DIR)
...
$(MINI2_RAW) $(MINI2_SPRITE_RAW) $(MINI2_SYM) &: $(ASM_SOURCES) $(TABLES) $(SPRITES) $(MUSIC_GEN) $(HUD_ICONS) $(TEXT_GEN) | $(MINI2_DIR)
...
# The briefings, packed. Five bits a character, sixty lines: about six hundred
# bytes of bank 7, which is what the cockpit's scanner runs in.
$(TEXT_GEN): $(SRC_DIR)/game/briefings.asm tools/packtext.py
	$(PYTHON) tools/packtext.py $< $@
