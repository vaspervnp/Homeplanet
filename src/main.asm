...
;  and clear of row A, which nothing at run time would notice.
    ...
    assert HUD_SQ_MARK_X + (SQUAD_MAX - 1) * HUD_SQ_MARK_STEP + (HUD_SQ_MARK_W + 3) / 4 <= HUD_SEL_X, "the squadron marks run into the selection"
    assert HUD_SQ_MARK_W <= 4 * HUD_SQ_MARK_STEP - 2, "the marks touch each other"
